...
function 直接通路(方式, 距离, 门禁集合, 起点, 终点, 终点位置x, 终点位置y,){
	let that = this;
	//通路方式有"上""下""左""右""跃迁"5种，方向仅仅用来指导移动动画。
	that.方式 = 方式;
	that.起点 = 起点;
	that._终点 = 终点;//终点是隐藏属性，不应直接使用
	//在格子初始化的时候，会顺带初始化直接通路。
	//但是由于这时候格子周边的格子可能还没初始化，所以终点没有办法立即输入
	//所以只能输入终点位置作为代替。然后用一个"获取终点"方法来懒加载。
	that.终点位置x = 终点位置x;
	that.终点位置y = 终点位置y;
	that.距离 = 距离;
	that.门禁集合 = 门禁集合;
	if(that.原型方法已创建 !== true){
		直接通路.prototype.原型方法已创建 = true;
		直接通路.prototype.通路门禁检验 = function(权限码){
			let that = this;
		...
		直接通路.prototype.获取终点 = function(){
			let that = this;
			if(!that._终点){
				that._终点 = that.起点.所属地图.二维格子数组[that.终点位置x][that.终点位置y];
			}
			return that._终点;
		}
	}
...
function 格子(列号, 行号, 所属地图){
	...
	//直接通路集合，里面放的是这个格子与其他格子间的所有直接通路对象。一般总是有上下左右通路各一条，除边角外。
	...
	//这时候失效的原通路会放在失效直接通路集合里。
	that.失效直接通路集合 = new Set();
	//下面初始化直接通路。要注意，在这里的直接通路要用一种特殊的方式来初始化，
	//终点设为undefined，作为代替输入终点位置x和终点位置y
	//这是因为格子在创建的时候其周边格子未必已经创建了，因此这里选择以懒加载的方式来获取终点。
	//如果本格的列号大于0，认为初始化时它有向左的通路
	if(列号 > 0){
		that.直接通路集合.add(new 直接通路("左", 1, new Set(), that, undefined, 列号 - 1, 行号));
	}
	//如果本格的行号大于0，认为初始化时它有向上的通路
	if(行号 > 0){
		that.直接通路集合.add(new 直接通路("上", 1, new Set(), that, undefined, 列号, 行号 - 1));
	}
	//如果本格的列号小于列数 - 1，认为初始化时它有向右的通路
	if(列号 < that.所属地图.列数 - 1){
		that.直接通路集合.add(new 直接通路("右", 1, new Set(), that, undefined, 列号 + 1, 行号));
	}
	//如果本格的列号小于行数 - 1，认为初始化时它有向下的通路
	if(行号 < that.所属地图.行数 - 1){
		that.直接通路集合.add(new 直接通路("下", 1, new Set(), that, undefined, 列号, 行号 + 1));
	}
	...
	that.各角色权限广度优先格点通路搜索缓存映射集 = new Map();

	if (that.原型方法已创建 !== true){
		格子.prototype.原型方法已创建 = true;
		格子.prototype.获取中心像素点 = function(){
			let that = this;
		...
		格子.prototype.搜索指定距离内通达格子集合 = function(角色权限, 最大距离){
			let that = this;
		...
		格子.prototype.搜索目标格子通路列表 = function(角色权限, 目标格){
			let that = this;
			let 搜索缓存集 = that.各角色权限广度优先格点通路搜索缓存映射集.get(角色权限);
			if (搜索缓存集 === undefined){
				搜索缓存集 = new 广度优先格点通路搜索缓存(that, 角色权限);
			...
			return 搜索缓存集.搜索目标格子通路列表(目标格);
		}
	}
}

//这个对象是一个指定起点的，指定角色权限的通路搜索缓存。
function 广度优先格点通路搜索缓存(起点格, 角色权限){
	let that = this;
	...
	//这里的广度优先是迪杰斯特拉算法，迪杰斯特拉的通路信息只在本起点内生效，不可被其他起点复用。
	function 通路信息(总距离, 末端直接通路){
		let 通路信息that = this;
		通路信息that.总距离 = 总距离;
		//如果总距离是0，而末端直接通路却是null，说明目标格子就是起点格自己。
		通路信息that.末端直接通路 = 末端直接通路;
	}

	if(that.原型方法已创建 !== true){
		广度优先格点通路搜索缓存.prototype.原型方法已创建 = true;
		广度优先格点通路搜索缓存.prototype.重置 = function(){
			let that = this;
			...
			that.前线缓存映射集.set(that.通路起点, new 通路信息(0, null));
		...
		广度优先格点通路搜索缓存.prototype.刷新最小前线距离与前线候补格 = function(){
			let that = this;
			that.最小前线距离 = null;
		...
		广度优先格点通路搜索缓存.prototype.搜索缓存扩张 = function(停止扩张条件函数){
			let that = this;
			//首先检查版本是否过期
			...
			while(that.最小前线距离 !== null && !停止扩张条件函数()){
				...
				//搜索前线候补格的所有直接通路
				for(let 扩张直接通路 of that.前线候补格.直接通路集合){
					//先检查有没有权限，没有权限直接跳过
					if(!扩张直接通路.通路门禁检验(that.角色权限)){
						continue;
					}
					let 扩张目的格 = 扩张直接通路.获取终点();
					...
					if(前线已有目的通路信息 === undefined){
						that.前线缓存映射集.set(扩张目的格, new 通路信息(总距离, 扩张直接通路));
						continue;
					}
		...
		广度优先格点通路搜索缓存.prototype.搜索指定距离内通达格子集合 = function(最大距离){
			let that = this;
		...
		广度优先格点通路搜索缓存.prototype.搜索目标格子通路列表 = function(目标格){
			let that = this;
			that.搜索缓存扩张(function(){
...
function 地图纯容器(){
	...
	that.列数 = 16;
	that.行数 = 12;
	that.二维格子数组 = new Array(that.列数);
	for(let i = 0; i < that.列数; i += 1){
		that.二维格子数组[i] = new Array(that.行数);
		for(let j = 0; j < that.行数; j += 1){
			that.二维格子数组[i][j] = new 格子(i, j, that);
		}
	}
	...
	that.登场单个角色 = function(角色, 位置x, 位置y){
		...
		let 动画播放完毕后的行为 = function(){
			...
			登场角色实例.图标显示(true);
			全局.事件分发器.dispatchEvent("角色登场触发器");
		}
	}

	//
	that.移动角色并播放动画 = function(角色, 移动目标格){
		let 移动角色实例 = 全局.角色.获取实例(角色);
		let 移动起点格 = 移动角色实例.当前所在格;
		//获取从起点到终点的通路列表
		let 移动通路列表 = 移动起点格.搜索目标格子通路列表(移动角色实例.角色权限码, 移动目标格)
		if(移动通路列表.length === 0){
			return;
		}
		let 当前通路序号 = 0;
		...
		let 移动单帧脚本 = function(){
			...
			if(当前移动通路.距离 === 当前通路移动进度){
				//如果相等，立即归位
				let 当前移动通路终点中心像素点 = 当前移动通路.获取终点().获取中心像素点();
	...
	//非常重要！它确保移动中或行动中的角色不会被对其他角色的操作所打断
	//下一版设计可能不再需要它了。
	that.锁定操作角色 = null;
