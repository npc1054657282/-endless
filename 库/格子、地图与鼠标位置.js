...
class 直接通路 {
	constructor(方式, 距离, 门禁集合, 起点, 终点, 终点位置x, 终点位置y) {
		let that = this;
		//通路方式有"上""下""左""右""跃迁"5种，方向仅仅用来指导移动动画。
		that.方式 = 方式;
		that.起点 = 起点;
		//尽量不用class field的私有属性特性，依然用下划线标记约定私有变量。
		that._终点 = 终点;
	...
	get 终点() {
		let that = this;
		if (!that._终点) {
			that._终点 = that.起点.所属地图.二维格子数组[that._终点位置x][that._终点位置y];
		}
		return that._终点;
	}
}
...
//这里是事件函数库。主事件开发有什么好处？主要是方便随时修改监听内容。
//例如当地图上的一个结算需要调出UI对话框的时候，不需要知道全局的UI对话框对象是谁。
//只需要某个活跃的UI对话框已经绑定了某个事件操作，我发送这个事件就可以了。
//事件的添加器和移除器有待重写，以支持传递参数功能。
//我们自己做的含参事件分发器有一点特别之处，就是它会自动把监听执行体的this绑定给事件分发器自己。
//所以可以搞这种prototype式的函数
p.空闲点击 = function(){
	var that = this;
}
