...
function 地图纯容器(){
	...
	that.鼠标像素点 = null;//createjs.Point对象，指示鼠标在地图内的像素位置x和y。
	...
	//登场单个角色时，输入参数为uid。登场多个角色时，输入参数为一个uid列表。登场多个角色复杂太多，先不实现了。
	that.登场单个角色 = function(角色){
		...
		//最新版的createjs里这里的loop可以指定次数，但an版的createjs只是个布尔值，想确定循环次数只能多次播放。
		登场动画实例.play();
		let 动画播放完毕后的行为 = function(){
			console.log("动画播放完毕后的行为")
			全局.测试文本.color = "red"; 
		}
	}
