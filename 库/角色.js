...
class 角色 {
	constructor(角色类型) {
		...
		that.角色类型 = 角色类型;
		//权限标签可以通过一定规则转换为权限码。
		that.权限标签集 = Object.assign({}, 模组.角色大图鉴[角色类型].初始权限标签集);
		that.移动力 = 3,
	...
	get 权限码() {
		return cls.权限.权限标签集转换器(this.权限标签集);
	}
	get 当前所在格() {
...
角色.获取实例 = function(角色标识) {
	//角色标识有两可能，其一是bigint类型的uid，其二是string类型的角色剧情名，其三是我自己已经是角色实例了
	//为什么要有其三，因为有很多根据输入角色标识不同来获取实例的函数，如果每次输入一个实例都要额外判定太耗时间了
	//所以获取实例允许输入就是一个实例，输出我自己
	if(typeof 角色标识 === 'bigint'){
