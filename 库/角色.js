...
//角色大图鉴是一张静态表，包括各个角色类型（以角色名表示）的角色初始化参数。
...
class 角色 {
	constructor(名字) {
		...
		that.图标字典表 = {};
		for (let 角色图标元件名 of 角色大图鉴[名字].图标) {
		...
		that.当前所在格 = null;
		//移动操作留有撤销的余地，因此在完全确认前即使位置修改了，记忆位置也不会修改
		//一旦执行撤销操作，将使用记忆位置使角色归位。
		that.记忆所在格 = null;
	...
	图标切换(图标名) {
		...
		let 新图标 = that.图标字典表[图标名];
		//角色自初始化以来就存在当前图标名，当前图标名绝不可能为空
		let 旧图标 = that.图标字典表[that.当前图标名];
		旧图标.visible = false;
